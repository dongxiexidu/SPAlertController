...
//        alertController()
    ...
    func alertController() {
        let alert = SPAlertController.alertController(withTitle: "标题", message: "消息", preferredStyle: .alert, animationType: .fromBottom)
       // alert.needDialogBlur = true
    ...
    func sheetController() {
        ...
        let action3 = SPAlertAction.action(withTitle: "Cancel", style: .destructive) { (action) in
            print("点击了Cancel------")
        }
        alert.addAction(action: action1)
        alert.addAction(action: action2)
